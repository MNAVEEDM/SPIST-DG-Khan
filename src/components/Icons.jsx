...
/**
 * Merges a caller's props over the icon's own default size.
 *
 * The size has to be applied this way rather than as `width="16" {...props}`:
 * a caller passing `width={undefined}` — an unset entry in a config object,
 * say — would otherwise overwrite the default with nothing, leaving an SVG
 * that has a viewBox but no intrinsic size. Such an icon stretches to fill
 * whatever box contains it, which is how a 16px chevron ends up several
 * hundred pixels tall. Destructuring defaults ignore `undefined`, so the
 * fallback survives.
 */
const sized = (size, { width = size, height = size, ...rest }) => ({ width, height, ...rest });

export const ChevronDown = (props) => (
  <svg {...base} {...sized(16, props)}>
    <path d="m6 9 6 6 6-6" />
  </svg>
);
